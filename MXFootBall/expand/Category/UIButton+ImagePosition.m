...
-(NSString *)titleForState:(UIControlState)state andSubstringWithRange:(NSRange)range{
    return [[self titleForState:state] substringWithRange:range];
}
-(NSString *)titleForState:(UIControlState)state andFormIndex:(NSInteger)index{
    return [[self titleForState:state] substringWithRange:NSMakeRange(index, [self titleForState:state].length-index)];
}
