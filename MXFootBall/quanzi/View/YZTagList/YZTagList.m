...
- (void)setup
{
    ...
    _tagSelectFont = [UIFont systemFontOfSize:13];
    _tagSelectColor = [UIColor whiteColor];
    _tagSelectBackgroundColor = [UIColor redColor];
...
- (void)clickTag:(UIButton *)button
{
    ...
    if (tagsa != button.tag) {
       YZTagButton *btn = self.tagButtons[tagsa];
    ...
    [button setBackgroundColor:self.tagSelectBackgroundColor];
    button.titleLabel.font = self.tagFont;
    [button setTitleColor:self.tagSelectColor forState:UIControlStateNormal];
    tagsa = button.tag;
    NSLog(@"%ld",tagsa);
