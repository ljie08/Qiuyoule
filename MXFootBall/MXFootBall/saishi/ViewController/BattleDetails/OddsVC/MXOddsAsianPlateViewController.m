//
//  MXOddsAsianPlateViewController.m
//  MXFootBall
//
...
#import "MXOddsAsianPlateViewController.h"

#import "MXOddsTableViewCell.h"
#import "MXDAsiaModel.h"

@interface MXOddsAsianPlateViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MXOddsAsianPlateViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [UBT logTrace:@"residenceTimeIn" content:@"{\"VC\":\"赔率亚盘界面\"}"] ;
}
//赔率亚盘界面
- (void)viewDidDisappear:(BOOL)animated {
    [super viewDidDisappear:animated] ;
    
    [UBT logTrace:@"residenceTimeOut" content:@"{\"VC\":\"赔率亚盘界面\"}"] ;
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
//    NSString * matchID = @"2442832" ;
    
    NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
    
    [parameters setObject:matchID forKey:@"matchId"] ;
    [parameters setObject:@"asia" forKey:@"oddsType"];
    ...
    [[WebManager sharedManager]requestWithMethod:POST WithUrl:url WithParams:dic WithSuccessBlock:^(NSDictionary *dic) {
        ...
        if ([dic[@"code"] isEqualToString:@"0"]) {
            ...
            if ([dic[@"data"][@"odds"] count]) {
                weakSelf.dataArray = [MXDAsiaModel mj_objectArrayWithKeyValuesArray:dic[@"data"][@"odds"]] ;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        cell.firstInitialLabel.text = @"水位" ;
        cell.secondInitialLabel.text = @"盘口" ;
        cell.thirdInitialLabel.text = @"水位" ;
        ...
        cell.asiaModel = self.dataArray[indexPath.row - 1] ;
